...
class CommonController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        //产品的浏览记录
        $historyen = unserialize( $_COOKIE['historyen'] );    //当前已浏览过的商品二维数组
        $this->assign("historyen",$historyen);   //浏览商品show         【这三行主要跟商品浏览cookie有关】
    ...
    public function goodshistory( $goods_id ){
        ...
        $current = unserialize( $_COOKIE['historyen'] );   // 【当前已浏览过的商品  二维数组】
        ...
        if( $_COOKIE['historyen']=="" ){      //【如果一个商品也没看过则存入】
           ...
           $current[0]['goods_name']=$goodsResult['goods_name'];    //商品名称 goodsname为商品名称在数据库字段，下同
            ...
            cookie('historyen',serialize($current),array('expire'=>3600,'path'=>'/'));      // 【thinkphp特有的写cookie的函数方法】
          //  cookie('history',serialize($current));
            ...
            if(  $temp_s==0  )    //【如果没一样的则记录下来】
            {
                ...
                cookie('historyen',serialize($current),array('expire'=>3600,'path'=>'/'));
               // cookie('history',serialize($current));
            }
        }
    }
