...
class ServicesModel extends Model {
     ...
     * @return array 查询结果
     */
    public function getList($type='Services',$cids=0,$p=0){
        //准备查询条件
        $order = 'n.id desc';        //排序条件
        $field = 'c.name as servicecategory_name,n.servicecategory_id,n.date,n.id,n.name,n.on_sale,n.recommend';
        if($type=='services'){          //新闻列表页取数据时
        ...
        if($cids == 0){      //查找未分类的商品
            $where['n.servicecategory_id'] = 0;
        }elseif($cids > 0){  //查找分类ID数组
            $where['n.servicecategory_id'] = array('in',$cids);
        }
        //准备分页查询
        //查询数据
        ...
        $data = $this->alias('n')->join('flyin_servicecategory AS c ON c.id=n.servicecategory_id','LEFT')->field($field)
    ...
    public function getServices($where){
        //定义需要的字段
        $field = 'id,servicecategory_id,sn,name,date,on_sale,recommend,desc';
    ...
    public function delThumbFile($where){
        ...
        $path = "./Public/Uploads/service/big/$thumb";    //准备大图路径
        if(is_file($path)) unlink($path);         //删除大图文件
        $path = "./Public/Uploads/service/small/$thumb";  //准备小图路径
        if(is_file($path)) unlink($path);         //删除小图文件
        $path = "./Public/Uploads/service/yuan/$thumb";  //准备原图路径
    ...
    public function uploadThumb($upfile){
        //准备上传目录
        $file['temp'] = './Public/Uploads/service/temp/';                       //准备临时目录
        ...
        $file['path1'] = './Public/Uploads/service/big/'.$file['save'];   //大图路径
        $file['path2'] = './Public/Uploads/service/small/'.$file['save']; //小图路径
        $file['path3'] = './Public/Uploads/service/yuan/'.$file['save'];   //大图路径
        //创建保存目录
