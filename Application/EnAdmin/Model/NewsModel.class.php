...
/**
    //表单字段过滤
    protected $insertFields = 'sn,name,on_sale,recommend';
    protected $updateFields = 'sn,name,on_sale,recommend';
    //自动验证
    ...
    public function getList($type='news',$cids=0,$p=0){
        ...
        $field = 'c.name as newcategory_name,n.newcategory_id,n.custom_time,n.id,n.thumb,n.name,n.on_sale,n.recommend';
        if($type=='news'){          //新闻列表页取数据时
        ...
        if($cids == 0){      //查找未分类的商品
            $where['n.newcategory_id'] = 0;
        }elseif($cids > 0){  //查找分类ID数组
            $where['n.newcategory_id'] = array('in',$cids);
        }
        //查询数据
        //分页查询
        ...
        $data = $this->alias('n')->join('flyin_newcategory AS c ON c.id=n.newcategory_id','LEFT')->field($field)
    ...
    public function getNews($where){
        //定义需要的字段
        $field = 'id,newcategory_id,sn,name,custom_time,thumb,on_sale,recommend,desc';
    ...
    public function delThumbFile($where){
        ...
        $path = "./Public/Uploads/news/big/$thumb";    //准备大图路径
        if(is_file($path)) unlink($path);         //删除大图文件
        $path = "./Public/Uploads/news/small/$thumb";  //准备小图路径
    ...
    public function uploadThumb($upfile){
        //准备上传目录
        $file['temp'] = './Public/Uploads/news/temp/';                       //准备临时目录
        ...
        $file['path1'] = './Public/Uploads/news/big/'.$file['save'];   //大图路径
        $file['path2'] = './Public/Uploads/news/small/'.$file['save']; //小图路径
        //创建保存目录
