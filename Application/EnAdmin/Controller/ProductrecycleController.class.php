...
//回收站控制器
class ProductrecycleController extends CommonController{
    ...
    public function index(){
        ...
        $News = D('Products');
        //查询banner列表
        $data['products'] = $News->getList('recycle',-1,$p);
        //防止查询到空页
        if(empty($data['products']['data']) && $p > 0){
            $this->redirect('Productrecycle/index');
        }
        $data['p'] = $p;
        $this->assign($data);
        $this->display();
    ...
    public function rec(){
        ...
        //生成跳转地址
        $jump = U('Productrecycle/index',array('p'=>$p));
        //实例化模型
        $Products = M('Products');
        //检查表单令牌
        ...
        //将Banners取消删除
        if(false === $Products->where(array('id'=>$id))->save(array('recycle'=>'no'))){
