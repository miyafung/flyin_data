	...
	<div class="row-fluid">
		<div class="span12">
			<div class="top-button">
				相关操作：<a href="<?php echo U('Banners/index');?>" class="btn btn-primary">banner列表</a>
			...
			<div class="widget-box">
				...
				<div class="widget-content nopadding">
					<table class="table table-bordered table-striped">
						...
						<tbody>
							<!--循环输出列表 START-->
							<?php if(is_array($banners["data"])): foreach($banners["data"] as $key=>$v): ?><tr>
									<td class="t1"><img style="width: 50px" src="/Public/Uploads/banner/small/<?php echo ($v["thumb"]); ?>"></td>
									<td><?php if(empty($v["bannercategory_id"])): ?>未分类<?php else: echo ($v["bannercategory_name"]); endif; ?></td>
									<td><?php echo ($v["name"]); ?></td>
									<td><?php if(($v["on_sale"]) == "yes"): ?>是<?php else: ?>否<?php endif; ?></td>
									<td><?php if(($v["recommend"]) == "yes"): ?>是<?php else: ?>否<?php endif; ?></td><td>
									<a href="#" class="act-rec" data-id="<?php echo ($v["id"]); ?>">恢复</a>
...
<script>
	...
	$(".act-del").click(function(){
		if(confirm('确定要彻底删除吗？')){
			...
			$("#form").attr("action","<?php echo U('Bannerrecycle/del',array('p'=>$p));?>").submit();
		}
	});
	//恢复
	$(".act-rec").click(function(){
		$("#target_id").val($(this).attr("data-id"));
		$("#form").attr("action","<?php echo U('Bannerrecycle/rec',array('p'=>$p));?>").submit();
	});

    //清空回收站
    $(".act-delall").click(function(){
        if(confirm('确定要彻底删除吗？')){
            $("#form").attr("action","<?php echo U('Bannerrecycle/delall',array('p'=>$p));?>").submit();
        }
    });
</script></div>
