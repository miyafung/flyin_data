		...
		<div class="span12">
			...
			<div class="top-button">
				选择新闻分类：<select id="newcategory">
					...
					<?php if(is_array($newcategory)): foreach($newcategory as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(($v["id"]) == $cid): ?>selected<?php endif; ?>><?php echo str_repeat('— ',$v['level']); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
				</select>
				<a href="<?php echo U('News/add',array('cid'=>$cid));?>" class="btn btn-primary">添加新闻</a>
				<a href="<?php echo U('Newcategory/add');?>" class="btn btn-primary">添加分类</a>
			</div>
			...
			<div class="list full">
				<table class="table table-bordered table-striped">
					...
					<?php if(is_array($news["data"])): foreach($news["data"] as $key=>$v): ?><tr>
							<td class="t1"><img style="width: 50px" src="/Public/Uploads/news/small/<?php echo ($v["thumb"]); ?>"></td>
							<td>
							<?php if(empty($v["newcategory_id"])): ?><a href="<?php echo U('News/index','cid=0');?>" >未分类</a>
							<?php else: ?>
								<a href="<?php echo U('News/index',array('cid'=>$v['newcategory_id']));?>"><?php echo ($v["newcategory_name"]); ?></a><?php endif; ?>
						</td>
						...
						<a href="<?php echo U('News/edit',array('id'=>$v['id'],'cid'=>$v['newcategory_id'],'p'=>$p));?>">修改</a>　<a href="#" class="act-del" data-id="<?php echo ($v["id"]); ?>">删除</a>
...
<script>
	...
	$("#newcategory").change(function(){
		var url = "<?php echo U('News/index',array('cid'=>'_ID_'));?>";
	...
	function change_status(obj,field){
		...
		$("#form").attr("action","<?php echo U('News/change',array('p'=>$p,'cid'=>$cid));?>").submit();
	}
	//快捷操作-上架
	...
	$(".act-del").click(function(){
		if(confirm('确定要删除吗？')){
			...
			$("#form").attr("action","<?php echo U('News/del',array('p'=>$p,'cid'=>$cid));?>").submit();
		}
	});
</script></div>
