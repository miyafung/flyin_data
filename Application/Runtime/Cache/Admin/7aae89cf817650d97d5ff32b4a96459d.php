		...
		<div class="span12">
			...
			<div class="top-button">
				修改banner分类：<select id="bannercategory">
					...
					<?php if(is_array($bannercategory)): foreach($bannercategory as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(($v["id"]) == $cid): ?>selected<?php endif; ?>><?php echo str_repeat('— ',$v['level']); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
				</select>
				<a href="<?php echo U('Banners/index',array('cid'=>$cid,'p'=>$p));?>" class="btn btn-primary">返回banner列表</a>
			</div>
			...
			<div class="list auto">
				...
				<table class="t2 t4">
					<tr><th>banner名称：</th><td><input type="text" name="banner_name" value="<?php echo ($banners["banner_name"]); ?>" class="big"></td></tr>
					<tr><th>banner编号：</th><td><input type="text" name="sn" value="<?php echo ($banners["sn"]); ?>" ></td></tr>
					<tr><th>banner排序：</th><td><input type="text" name="rank" value="<?php echo ($banners["rank"]); ?>" ></td></tr>
					<tr><th>是否上线：</th><td><select name="on_sale">
						<option value="yes" <?php if(($banners["on_sale"]) == "yes"): ?>selected<?php endif; ?>>是</option>
							<option value="no" <?php if(($banners["on_sale"]) == "no"): ?>selected<?php endif; ?>>否</option>
					</select></td></tr>
					<tr><th>首页推荐：</th><td><select name="recommend">
						<option value="yes" <?php if(($banners["recommend"]) == "yes"): ?>selected<?php endif; ?>>是</option>
						<option value="no" <?php if(($banners["recommend"]) == "no"): ?>selected<?php endif; ?>>否</option>
					</select></td></tr>
					<tr><th>修改banner：</th><td><input type="file" name="thumb" /></td></tr>
					<tr><th>当前banner：</th><td>
						<?php if(empty($banners["thumb"])): ?><img src="/Public/Common/img/preview.jpg">
						<?php else: ?>
							<img src="/Public/Uploads/banner/small/<?php echo ($banners["thumb"]); ?>"><?php endif; ?>
					</td>
					</tr>
				</table>
...
<script>
    $(function(){
        ...
        UM.getEditor("myEditor",{
            "imageUrl":"<?php echo U('Banners/uploadImage');?>", //图片上传提交地址
            "imagePath":"/Public/Uploads/banner/desc/"  //图片显示地址
        });
    });
</script>
					<script type="text/plain" id="myEditor" name="desc"><?php echo ($banners["desc"]); ?></script>
				</div>
...
<script>
	...
	$("#bannercategory").change(function(){
		var url = "<?php echo U('Banners/edit',array('id'=>$id,'cid'=>'_ID_','p'=>$p));?>";
