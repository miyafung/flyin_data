...
<div id="content">
	...
	<div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> 方案&支持分类</a> <a href="#" class="current">方案&支持分类</a> </div>
	<h1>方案&支持分类</h1>
</div>
...
<div class="top-button">
	相关操作：<a href="<?php echo U('Servicecategory/index');?>" class="btn btn-primary btn-lg">方案&支持分类列表</a>
</div>
...
<div class="list auto">
	...
	<table class="t2 t3">
		<tr><th>上级分类：</th><td>
			<select name="pid" id="servicecategory">
				<option value="0">顶级分类</option>
				<?php if(is_array($servicecategory)): foreach($servicecategory as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo str_repeat('— ',$v['level']); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
			</select>
		</td></tr>
		<tr><th>分类名称：</th><td><input type="text" name="name"></td></tr>
	</table>
	<div class="btn">
		<input type="submit"class="btn btn-primary btn-lg" value="添加分类">
		<input type="submit"class="btn btn-primary btn-lg" value="添加并返回" name="return">
	</div>
	</form>
</div>
<?php if(isset($pid)): ?><script> $("#servicecategory").val(<?php echo ($pid); ?>); </script><?php endif; ?></div>
