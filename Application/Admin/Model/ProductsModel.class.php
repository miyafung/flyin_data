...
class ProductsModel extends Model {
    ...
    public function getList($type='products',$cids=0,$p=0){
        //准备查询条件
        $order = 'n.products_id desc';        //排序条件
        $field = 'c.name as productcategory_name,n.productcategory_id,n.products_id,n.img_center,n.products_name,n.on_sale,n.recommend';
        if($type=='products'){          //新闻列表页取数据时
        ...
        if($cids == 0){      //查找未分类的商品
            $where['n.productcategory_id'] = 0;
        }elseif($cids > 0){  //查找分类ID数组
            $where['n.productcategory_id'] = array('in',$cids);
        }
        //准备分页查询
        ...
        $data = $this->alias('n')->join('flyin_productcategory AS c ON c.id=n.productcategory_id','LEFT')->field($field)
    ...
    public function getProducts($where){
        //定义需要的字段
        $field = 'products_id,productcategory_id,sn,name,img_center,on_sale,recommend,desc';
    ...
    public function delThumbFile($where){
        //取出原图文件名
        $thumb = $this->where($where)->getField('img_center');
        if(!$thumb) return ;  //商品图片不存在时直接返回
        $path = "./Public/Uploads/PP/big/$thumb";    //准备大图路径
        if(is_file($path)) unlink($path);         //删除大图文件
        $path = "./Public/Uploads/PP/small/$thumb";  //准备小图路径
        if(is_file($path)) unlink($path);         //删除小图文件
        $path = "./Public/Uploads/PP/yuan/$thumb";  //准备原图路径
    ...
    public function uploadThumb($upfile){
        //准备上传目录
        $file['temp'] = './Public/Uploads/PP/temp/';                       //准备临时目录
